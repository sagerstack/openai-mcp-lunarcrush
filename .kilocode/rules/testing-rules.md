...
### Rules
3. There are always 4 kinds of tests- poc, unit, integration and e2e
...
2. **POC Test First** Before writing any source code, start by writing a poc (proof-of-concept) test. The objective of this test is to conduct a simple verification of functional scope and most importantly, external connectivity. For example, write a poc test to verify you can fetch data from all the lunarcrush API endpoints that is mentioned in the implementation plan. If this test fails, stop further implementation.

3. Follow **Test-Driven Development** when writing code. Follow the RED->GREEN->REFACTOR methodology 
...
4. **Annotate** tests with "unit", "integration", "poc" and "e2e" to mark tests 
